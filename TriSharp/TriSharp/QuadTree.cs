namespace TriSharp
{
    public class QuadTree
    {
        ...
        readonly List<Vertex> _items;

        public QuadTree(Rect bounds, int numPoints)
        {
            ...
            Bounds = bounds;
            _items = new List<Vertex>(numPoints);
        }
        ...
        public QuadTree(Rect bounds, int maxDepth = 10, int maxItems = 8)
        {
            ...
            root = new QuadNode(bounds, 0, maxDepth, maxItems);
            Bounds = bounds;
            _items = new List<Vertex>();
        }

        public Rect Bounds { get; }
        public List<Vertex> Items => _items;
        ...
        public List<Vertex> Query(Rect area)
        {
        ...
        public Vertex? TryGet(double x, double y, double precision = 1e-10)
        {
        ...
        class QuadNode
        {
            ...
            public Vertex? TryGet(double x, double y, double eps = 1e-10)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        Vertex? result = _children[i].TryGet(x, y, eps);
            ...
            public void Query(Rect area, List<Vertex> results)
            {
